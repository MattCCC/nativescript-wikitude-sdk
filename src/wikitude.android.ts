...
export class Wikitude extends Common implements IWikitudeFunctions {
  ...
  private defaultFeatures: number =
    Features.GeoTracking |
    Features.ImageTracking |
    Features.InstantTracking |
    Features.ObjectTracking;
  ...
  createNativeView() {
    ...
    if (this.features) {
      ...
      this.config.setFeatures(
        com.wikitude.architect.ArchitectStartupConfiguration.Features.Geo |
          com.wikitude.architect.ArchitectStartupConfiguration.Features
            .ImageTracking |
          com.wikitude.architect.ArchitectStartupConfiguration.Features
            .InstantTracking |
          com.wikitude.architect.ArchitectStartupConfiguration.Features
    ...
    this._android = new com.wikitude.architect.ArchitectView(
  ...
  initNativeView() {
    ...
    try {
      ...
      this._android.addArchitectJavaScriptInterfaceListener(
        this.jsonObjectListener
      );
    } catch (e) {
  ...
  hasFeature(feature: number): boolean {
    if (feature === null) {
      return com.wikitude.architect.ArchitectView.isDeviceSupported(
        this.currentActivity
      );
    }

    const missingFeatures: any = com.wikitude.architect.ArchitectView.isDeviceSupported(
      this.currentActivity,
      feature
    );
  ...
  loadUrl(urlString: string, features?: number) {
    ...
    if (urlString.indexOf("~/") === 0) {
      urlString = urlString.replace(
        "~/",
        `file://${knownFolders.currentApp().path}/`
      );
    }
  ...
  captureScreen(captureWebViewContent: boolean = false) {
    ...
    if (captureWebViewContent) {
      captureMode =
        com.wikitude.architect.ArchitectView.CaptureScreenCallback
  ...
  switchCamera() {
    ...
    if (
      ...
      com.wikitude.common.camera.CameraSettings.CameraPosition.FRONT
    ) {
      newCameraPosition =
        com.wikitude.common.camera.CameraSettings.CameraPosition.BACK;
    } else if (
      currentCameraPosition ===
      com.wikitude.common.camera.CameraSettings.CameraPosition.BACK
    ) {
      ...
      newCameraPosition =
        com.wikitude.common.camera.CameraSettings.CameraPosition.DEFAULT;
    }
...
//     this.log(
//       this.isLicensed
//         ? "But we are already licensed so we are ignoring it!"
//         : "We are setting the license now!"
//     );
...
export class LocationProvider<Owner> {
  ...
  public LocationManager: android.location.LocationManager = this.context.getSystemService(
  ...
  public LocationListener: android.location.LocationListener = new android.location.LocationListener(
    this.Listener
  );
  ...
  isBetterLocation(location: android.location.Location): boolean {
    ...
    const accuracyDelta =
      location.getAccuracy() - this.currentLocation.getAccuracy();
    ...
    const isFromSameProvider = this.isSameProvider(
      location.getProvider(),
      this.currentLocation.getProvider()
    );
  ...
  private setupEventListener(provider: string) {
    ...
    const lastKnownPosition = this.LocationManager.getLastKnownLocation(
      provider
    );
