...
declare class SocketIOClient extends NSObject {
	...
	didConnectToNamespace(namespace_: string): void;
	...
	emitWithCompletion(event: string, items: NSArray<any> | any[], completion: () => void): void;
...
declare class SocketManager extends NSObject implements SocketManagerSpec {
	...
	randomizationFactor: number; // inherited from SocketManagerSpec

	reconnectWait: number; // inherited from SocketManagerSpec

	reconnectWaitMax: number; // inherited from SocketManagerSpec
	...
	emitAllWithItems(event: string, items: NSArray<any> | any[]): void;
